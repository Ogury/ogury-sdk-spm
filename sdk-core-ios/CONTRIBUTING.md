...
The project is relying of **TravisCI** to perform the continuous integration process. **TravisCI** is reponsible for uploading the artefacts to the right platforms (**GitHub**, **Artifactory**, **Amazon S3**).

In order to perform a release locally, you can use the following command:

`bundle exec fastlane deploy_artefact environment:"ENVIRONMENT" version:"VERSION"`
...
If you want to perform a local release for testing purpose, you can use the same command as above and append the `dry_run:true` parameter:

`bundle exec fastlane deploy_artefact environment:"ENVIRONMENT" version:"VERSION" dry_run:true`

This will perform the exact same step as a CI release without the artefacts uploading.
