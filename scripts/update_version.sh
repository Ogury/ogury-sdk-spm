...
if [[ -z "${CONFIG_FOLDER:-}" || -z "${LOCAL_XCCONFIG:-}" || -z "${OGURY_MODE:-}" || -z "${OGURY_FRAMEWORK_NAME:-}" || -z "${OGURY_LABEL:-}" ]]; then
...
if [ -z "$APP_PLIST" ]; then
  echo "❌ ERROR: INFOPLIST_FILE is not set."
  exit 1
fi
if [ ! -f "$APP_PLIST" ]; then
...
if [ -z "$OGURY_VERSION" ]; then
  echo "❌ Failed to extract OgurySDK version"
  exit 1
fi
echo "🎯 OgurySDK version: $OGURY_VERSION (from pod: $OGURY_POD_NAME)"

# Mediation SDK version
MEDIATION_VERSION=""
if [[ -n "$MEDIATION_FRAMEWORK_NAME" && -n "$MEDIATION_LABEL" ]]; then
  if [ ! -f "$PODFILE_LOCK" ]; then
    echo "❌ Podfile.lock not found: $PODFILE_LOCK"
    exit 1
  fi
  MEDIATION_VERSION=$(grep -E "^[[:space:]]*-[[:space:]]*${MEDIATION_FRAMEWORK_NAME}[[:space:]]*\(.*\)" "$PODFILE_LOCK" \
    ...
    | head -n1)
  if [ -z "$MEDIATION_VERSION" ]; then
    echo "❌ Failed to extract mediation version for $MEDIATION_FRAMEWORK_NAME"
    exit 1
  fi
...
if [ -n "$CUSTOM_VERSION" ]; then
  ...
  VERSION_STRING="${APP_VERSION}+${OGURY_LABEL}.${OGURY_VERSION}"
