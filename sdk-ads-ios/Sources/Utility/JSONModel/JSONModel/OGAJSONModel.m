...
#import "OGAJSONModelClassProperty.h"

#pragma mark - associated objects names
static const char *kOGAMapperObjectKey;
static const char *kOGAClassPropertiesKey;
static const char *kOGAClassRequiredPropertyNamesKey;
static const char *kOGAIndexPropertyNameKey;

#pragma mark - class static variables
static NSArray *allowedJSONTypes = nil;
static NSArray *allowedPrimitiveTypes = nil;
static OGAJSONValueTransformer *valueTransformer = nil;
static Class JSONModelClass = NULL;

#pragma mark - model cache
static OGAJSONKeyMapper *globalKeyMapper = nil;

#pragma mark - JSONModel implementation
@implementation OGAJSONModel {
...
+ (void)load {
    ...
    dispatch_once(&once, ^{
        ...
        @autoreleasepool {
            ...
            valueTransformer = [[OGAJSONValueTransformer alloc] init];
...
- (void)__setup__ {
    ...
    if (!objc_getAssociatedObject(self.class, &kOGAClassPropertiesKey)) {
        [self __inspectProperties];
    }

    // if there's a custom key mapper, store it in the associated object
    id mapper = [[self class] keyMapper];
    if (mapper && !objc_getAssociatedObject(self.class, &kOGAMapperObjectKey)) {
...
- (instancetype)initWithData:(NSData *)data error:(NSError *__autoreleasing *)err {
    // check for nil input
    if (!data) {
        if (err) *err = [OGAJSONModelError errorInputIsNil];
        return nil;
    }
    // read the json
    OGAJSONModelError *initError = nil;
    id obj = [NSJSONSerialization JSONObjectWithData:data
                                             options:kNilOptions
                                               error:&initError];

    if (initError) {
        if (err) *err = [OGAJSONModelError errorBadJSON];
        return nil;
    }
...
- (id)initWithString:(NSString *)string error:(OGAJSONModelError **)err {
    OGAJSONModelError *initError = nil;
...
- (id)initWithString:(NSString *)string usingEncoding:(NSStringEncoding)encoding error:(OGAJSONModelError **)err {
    // check for nil input
    if (!string) {
        if (err) *err = [OGAJSONModelError errorInputIsNil];
        return nil;
...
- (id)initWithDictionary:(NSDictionary *)dict error:(NSError **)err {
    ...
    if (![dict isKindOfClass:[NSDictionary class]]) {
        if (err) *err = [OGAJSONModelError errorInvalidDataWithMessage:@"Attempt to initialize JSONModel object using initWithDictionary:error: but the dictionary parameter was not an 'NSDictionary'."];
        return nil;
    }
    ...
    if (!self) {
        ...
        if (err) *err = [OGAJSONModelError errorModelIsInvalid];
        return nil;
    }
...
- (OGAJSONKeyMapper *)__keyMapper {
    // get the model key mapper
    return objc_getAssociatedObject(self.class, &kOGAMapperObjectKey);
}

- (BOOL)__doesDictionary:(NSDictionary *)dict matchModelWithKeyMapper:(OGAJSONKeyMapper *)keyMapper error:(NSError **)err {
    ...
    if (keyMapper || globalKeyMapper) {
        ...
        for (OGAJSONModelClassProperty *property in [self __properties__]) {
    ...
    if (![requiredProperties isSubsetOfSet:incomingKeys]) {
        ...
        if (err) *err = [OGAJSONModelError errorInvalidDataWithMissingKeys:requiredProperties];
        return NO;
    }
...
- (NSString *)__mapString:(NSString *)string withKeyMapper:(OGAJSONKeyMapper *)keyMapper {
    if (keyMapper) {
        // custom mapper
...
- (BOOL)__importDictionary:(NSDictionary *)dict withKeyMapper:(OGAJSONKeyMapper *)keyMapper validation:(BOOL)validation error:(NSError **)err {
    ...
    for (OGAJSONModelClassProperty *property in [self __properties__]) {
        ...
        if (OGAisNull(jsonValue)) {
            ...
            if (err) {
                ...
                OGAJSONModelError *dataErr = [OGAJSONModelError errorInvalidDataWithMessage:msg];
        ...
        if (property) {
            ...
            if ([self __isJSONModelSubClass:property.type]) {
                ...
                if (
                    (![jsonValue isKindOfClass:property.type] && !OGAisNull(jsonValue)) ||
                    ...
                    Class sourceClass = [OGAJSONValueTransformer classByResolvingClusterClasses:[jsonValue class]];
                    ...
                    if (foundCustomTransformer) {
                        ...
                        if (err) {
                            ...
                            OGAJSONModelError *dataErr = [OGAJSONModelError errorInvalidDataWithTypeMismatch:msg];
                            *err = [dataErr errorByPrependingKeyPathComponent:property.name];
                        }
                        return NO;
                    }
...
- (NSMutableSet *)__requiredPropertyNames {
    ...
    NSMutableSet *classRequiredPropertyNames = objc_getAssociatedObject(self.class, &kOGAClassRequiredPropertyNamesKey);

    if (!classRequiredPropertyNames) {
        classRequiredPropertyNames = [NSMutableSet set];
        [[self __properties__] enumerateObjectsUsingBlock:^(OGAJSONModelClassProperty *p, NSUInteger idx, BOOL *stop) {
        ...
        objc_setAssociatedObject(
            self.class,
            &kOGAClassRequiredPropertyNamesKey,
...
- (NSArray *)__properties__ {
    ...
    NSDictionary *classProperties = objc_getAssociatedObject(self.class, &kOGAClassPropertiesKey);
    ...
    classProperties = objc_getAssociatedObject(self.class, &kOGAClassPropertiesKey);
...
- (void)__inspectProperties {
    ...
    while (class != [OGAJSONModel class]) {
        ...
        for (unsigned int i = 0; i < propertyCount; i++) {
            OGAJSONModelClassProperty *p = [[OGAJSONModelClassProperty alloc] init];
...
#pragma GCC diagnostic pop
            ...
            if (p) {
                ...
                for (Class type in allowedJSONTypes) {
                    NSString *class = NSStringFromClass([OGAJSONValueTransformer classByResolvingClusterClasses:type]);
...
- (id)__transform:(id)value forProperty:(OGAJSONModelClassProperty *)property error:(NSError **)err {
        ...
        if ([property.type isSubclassOfClass:[NSArray class]]) {
            ...
            if (![[value class] isSubclassOfClass:[NSArray class]]) {
                if (err != nil) {
                    ...
                    OGAJSONModelError *typeErr = [OGAJSONModelError errorInvalidDataWithTypeMismatch:mismatch];
            ...
            // one shot conversion
            OGAJSONModelError *arrayErr = nil;
...
- (id)__reverseTransform:(id)value forProperty:(OGAJSONModelClassProperty *)property {
    Class protocolClass = NSClassFromString(property.protocol);
    ...
    if ([self __isJSONModelSubClass:protocolClass]) {
        ...
        if (property.type == [NSArray class] || property.type == [NSMutableArray class]) {
            ...
            for (NSObject<OGAAbstractJSONModelProtocol> *model in (NSArray *)value) {
        ...
        if (property.type == [NSDictionary class] || property.type == [NSMutableDictionary class]) {
            ...
            for (NSString *key in [(NSDictionary *)value allKeys]) {
                id<OGAAbstractJSONModelProtocol> model = value[key];
...
- (BOOL)__customSetValue:(id<NSObject>)value forProperty:(OGAJSONModelClassProperty *)property {
    NSString *class = NSStringFromClass([OGAJSONValueTransformer classByResolvingClusterClasses:[value class]]);
...
- (BOOL)__customGetValue:(id *)value forProperty:(OGAJSONModelClassProperty *)property {
...
- (NSDictionary *)toDictionaryWithKeys:(NSArray<NSString *> *)propertyNames {
    ...
    for (OGAJSONModelClassProperty *p in properties) {
        // skip if unwanted
        ...
        if ([value isKindOfClass:JSONModelClass]) {
            // recurse models
            value = [(OGAJSONModel *)value toDictionary];
            [tempDictionary setValue:value forKeyPath:keyPath];
...
+ (NSMutableDictionary *)dictionaryOfModelsFromDictionary:(NSDictionary *)dictionary error:(NSError **)err {
    ...
    for (NSString *key in dictionary.allKeys) {
        ...
        if ([object isKindOfClass:NSDictionary.class]) {
            ...
            if (err) {
                *err = [OGAJSONModelError errorInvalidDataWithTypeMismatch:@"Only dictionaries and arrays are supported"];
            }
            return nil;
        }
    }
...
+ (NSMutableArray *)arrayOfDictionariesFromModels:(NSArray *)array {
    ...
    for (id<OGAAbstractJSONModelProtocol> object in array) {
...
+ (NSMutableDictionary *)dictionaryOfDictionariesFromModels:(NSDictionary *)dictionary {
    // bail early
    if (OGAisNull(dictionary)) return nil;

    NSMutableDictionary *output = [NSMutableDictionary dictionaryWithCapacity:dictionary.count];

    for (NSString *key in dictionary.allKeys) {
        id<OGAAbstractJSONModelProtocol> object = dictionary[key];
        id obj = [object toDictionary];
        if (!obj) return nil;
        output[key] = obj;
    }
...
- (NSString *)indexPropertyName {
    ...
    return objc_getAssociatedObject(self.class, &kOGAIndexPropertyNameKey);
}
...
- (NSString *)description {
    ...
    for (OGAJSONModelClassProperty *p in [self __properties__]) {
...
+ (void)setGlobalKeyMapper:(OGAJSONKeyMapper *)globalKeyMapperParam {
