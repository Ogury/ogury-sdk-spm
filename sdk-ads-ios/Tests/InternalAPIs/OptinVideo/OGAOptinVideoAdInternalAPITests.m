...
- (void)testShowAdInViewController {
    ...
    __block NSArray<id<OGAConditionChecker>> *additionalConditionals;
    OCMVerify([self.adManager show:self.internalAPI.sequence
              additionalConditions:[OCMArg checkWithBlock:^BOOL(id obj) {
                  additionalConditionals = obj;
                  return YES;
              }]]);
    XCTAssertEqual(additionalConditionals.count, 1);
    XCTAssertEqual(additionalConditionals[0], self.anotherAdInOverlayStateChecker);
}
