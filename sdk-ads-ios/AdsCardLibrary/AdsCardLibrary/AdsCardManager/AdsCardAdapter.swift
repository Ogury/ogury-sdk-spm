...
protocol AdAdapterFormat: Identifiable where ID == UUID {
    ...
    var options: AdAdapterFormatOptions { get }
}

protocol AdAdapterFormatOptions {
    /// - returns: return `true` if rtbTestMode (i.e. add `test:0` to bidding request) can be used on the server side, `false` if not
    var enableRtbTestMode: Bool { get }
}

protocol AdsCardAdapter {
