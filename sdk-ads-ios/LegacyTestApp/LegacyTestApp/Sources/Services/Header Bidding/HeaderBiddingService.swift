...
struct HeaderBiddingService {
    ...
    static let fakeBody = """
        {
          "app": {
            "bundle": "co.ogury.Test-Application",
            "cat": [
              "IAB1",
              "IAB1-1",
              "IAB3",
    ...
    static func retrieveAdMarkup(assetKey: String, adUnitId: String, country: String?, campaignId: String?, creativeId: String?, dspCreativeId: String? = nil, dspRegion: String? = nil, completionHandler: @escaping HeaderBiddingServiceCompletionHandler) {
        guard let headerBiddingURL = AdConfigController.shared.headerBiddingURL(), let URL = URL(string: headerBiddingURL) else {
    ...
    private static func buildGeoObject(with country: String?) -> String {
        switch country {
            case "USA":
    ...
    private static func handle(data: Data?, response: URLResponse?, error: Error?, completionHandler: HeaderBiddingServiceCompletionHandler) {
        if let error = error {
            completionHandler(.failure(.networkError(subError: error)))
            return
        }
