...
extension UIColor {
    ...
    fileprivate func withBrightness(brightness: CGFloat) -> UIColor {
        var hue: CGFloat = 0, saturation: CGFloat = 0, brigh: CGFloat = 0, alpha: CGFloat = 0
        if getHue(&hue, saturation: &saturation, brightness: &brigh, alpha: &alpha) {
            brigh += (brightness - 1.0)
            brigh = max(min(brigh, 1.0), 0.0)
            return UIColor(hue: hue, saturation: saturation, brightness: brigh, alpha: alpha)
        }
        return self
    }
